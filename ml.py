...
ty = pickle.load(open(folder + "train_labels", "rb"))
vy = pickle.load(open(folder + "val_labels", "rb"))
...
outstr = "Epoch,TrainLoss,ValLoss,TrainF1,ValF1\n"
...
for epoch in range(Epochs):
    ...
    for i, (x_batch, y_batch) in enumerate(trainloader):
        ...
        optimizer.step()
        optimizer.zero_grad()
        f1 += calc_f1(y.tolist(), y_pred.data.tolist())
        c = i + 1
    tloss = tloss / c
    tf1 = f1 / c
    ...
    for i, (x_batch, y_batch) in enumerate(valloader):
        ...
        val_f1 += calc_f1(y.tolist(), y_pred.data.tolist())
        vloss += float(loss)
        c = i + 1
        # print(f"Loss at epoch {epoch}, Iter {i} is: {loss}")
    vloss = vloss / c
    ...
    outstr += f"{epoch+1},{tloss},{vloss},{tf1},{vf1}"
    outstr += "\n"
